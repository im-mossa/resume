...
export default function SortControl() {
    ...
    return (
        <div className="flex gap-2 items-center">
            ...
            <select value={sort_by} onChange={(e) => update(e.target.value, order)} className="border rounded px-2 py-1">
                {sortOptions.map((o) => (
                    <option key={o.key} value={o.key}>{o.label}</option>
                ))}
            </select>
            <select value={order} onChange={(e) => update(sort_by, e.target.value)} className="border rounded px-2 py-1">
